...
class BOVWIndexer(BaseIndexer):
    def __init__(self, fvectorSize, dbPath, estNumImages=500, maxBufferSize=500, dbResizeFactor=2,verbose=True):
        ...
        #open the HDF5 database for writing, initialize the datasets within the group
        ...
        #store the feature vector size of the bovw, then initialize the document 
        #fequency counts to be accumulated and
    ...
    def add(self, hist):
        #update the BOVW buffer and teh document frequency counts
    ...
    def df(self, method=None):
        if method == "idf":
            ...
            return np.log10(self.totalImages / (1.0 + self._df))
